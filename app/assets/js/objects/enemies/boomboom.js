...
class BoomBoom extends Enemy {
  constructor(data) {
    super(data);

    this.scene = this.data.scene;
	if ((this.data.flags & 0x2) == 0x2) {
		this.spriteOffset = this.scene.spriteSheetData.enemies.boom_boom.wings;
	} else {
		this.spriteOffset = this.scene.spriteSheetData.enemies.boom_boom.default;
	}
  }
	...
	draw() {
      ...
      if (scale == 1) {
        this.canvasContext.drawImage(
  				...
  				(this.scene.canvas.height - this.data.position.y - heightPositionAdjustment - spriteHeightAdjustment - 0.6775),
        ...
        this.canvasContext.drawImage(
  				...
  				modifierOffset.height,
  				this.data.position.x - widthPositionAdjustment - spriteWidthAdjustment + (1*scale - 0.5),
  				(this.scene.canvas.height - this.data.position.y - heightPositionAdjustment - spriteHeightAdjustment - 0.365),
  				this.spriteOffset.width/32,
  				this.spriteOffset.height/32
  			);
      }
